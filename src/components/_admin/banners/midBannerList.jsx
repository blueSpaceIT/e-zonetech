...
export default function MidBannerList() {
  const { data, isLoading, refetch } = useQuery(['mid-banners'], () => api.getMidBanners(), { enabled: true });
  ...
  const handleCreateOrUpdate = async () => {
    try {
      if (editing) {
        await api.updateMidBanner(editing._id, form);
      } else {
        await api.createMidBanner(form);
      }
      setOpen(false);
      await refetch();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Delete this mid banner?')) return;
    try {
      await api.deleteMidBanner(id);
      await refetch();
    } catch (err) {
      console.error(err);
      alert('Delete failed');
  ...
  return (
    <Box sx={{ mt: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        <Typography variant="h6">Mid Banners</Typography>
        ...
        {isLoading ? (
          ...
          banners.map((b) => (
            <Grid item xs={12} sm={6} md={4} key={b._id}>
              <Card>
                {b.image && <CardMedia component="img" height="140" image={b.image} alt="mid banner" />}
                <CardContent>
          ...
          <Grid item xs={12}>
            <Typography>No mid banners found.</Typography>
          </Grid>
        )}
      </Grid>

      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{editing ? 'Edit Mid Banner' : 'Add Mid Banner'}</DialogTitle>
        <DialogContent>
          <Box sx={{ mt: 2 }}>
