...
export default function BannerFiveList() {
  const { data, isLoading, refetch } = useQuery(['banner-five'], () => api.getBannerFive(), { enabled: true });
  ...
  const handleCreateOrUpdate = async () => {
    try {
      if (editing) {
        await api.updateBannerFive(editing._id, form);
      } else {
        await api.createBannerFive(form);
      }
      setOpen(false);
      await refetch();
    } catch (err) {
  ...
  return (
    <Box sx={{ mt: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        <Typography variant="h6">Banner Five</Typography>
        ...
        {isLoading ? (
          ...
          banners.map((b) => (
            <Grid item xs={12} sm={6} md={4} key={b._id}>
              <Card>
                {b.image && <CardMedia component="img" height="140" image={b.image} alt="banner five" />}
                <CardContent>
          ...
          <Grid item xs={12}>
            <Typography>No banner five found.</Typography>
          </Grid>
        )}
      </Grid>

      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{editing ? 'Edit Banner Five' : 'Add Banner Five'}</DialogTitle>
        <DialogContent>
          <Box sx={{ mt: 2 }}>
