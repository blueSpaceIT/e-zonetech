...
export default function BannerFourList() {
  const { data, isLoading, refetch } = useQuery(['banner-four'], () => api.getBannerFour(), { enabled: true });
  ...
  const handleCreateOrUpdate = async () => {
    try {
      if (editing) {
        await api.updateBannerFour(editing._id, form);
      } else {
        await api.createBannerFour(form);
      }
      setOpen(false);
      await refetch();
    } catch (err) {
  ...
  return (
    <Box sx={{ mt: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        <Typography variant="h6">Banner Four</Typography>
        ...
        {isLoading ? (
          ...
          banners.map((b) => (
            <Grid item xs={12} sm={6} md={4} key={b._id}>
              <Card>
                {b.image && <CardMedia component="img" height="140" image={b.image} alt="banner four" />}
                <CardContent>
          ...
          <Grid item xs={12}>
            <Typography>No banner four found.</Typography>
          </Grid>
        )}
      </Grid>

      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{editing ? 'Edit Banner Four' : 'Add Banner Four'}</DialogTitle>
        <DialogContent>
          <Box sx={{ mt: 2 }}>
