...
export default function SideBannerList() {
  const { data, isLoading, refetch } = useQuery(['side-banners'], () => api.getSideBanners(), { enabled: true });
  ...
  const handleCreateOrUpdate = async () => {
    try {
      if (editing) {
        await api.updateSideBanner(editing._id, form);
      } else {
        await api.createSideBanner(form);
      }
      setOpen(false);
      await refetch();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Delete this side banner?')) return;
    try {
      await api.deleteSideBanner(id);
      await refetch();
    } catch (err) {
      console.error(err);
      alert('Delete failed');
  ...
  return (
    <Box sx={{ mt: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        <Typography variant="h6">Side Banners</Typography>
        ...
        {isLoading ? (
          ...
          banners.map((b) => (
            <Grid item xs={12} sm={6} md={4} key={b._id}>
              <Card>
                {b.image && <CardMedia component="img" height="140" image={b.image} alt="side banner" />}
                <CardContent>
          ...
          <Grid item xs={12}>
            <Typography>No side banners found.</Typography>
          </Grid>
        )}
      </Grid>

      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{editing ? 'Edit Side Banner' : 'Add Side Banner'}</DialogTitle>
        <DialogContent>
          <Box sx={{ mt: 2 }}>
