...
export default function GenderMain({ ...props }) {
  ...
  return (
    <>
      ...
      <Grid container>
        {genders?.filter(Boolean).map((v, i) => {
          const keyName = String(v || 'others').toLowerCase();
          const icon = icons[keyName] || icons.others;
          return (
            <Grid key={`${keyName}-${i}`} item xs={6}>
              <FormGroup>
                <FormControlLabel
                  ...
                  control={<Checkbox {...label} icon={icon} checkedIcon={icon} />}
                  label={keyName}
                />
              </FormGroup>
            </Grid>
